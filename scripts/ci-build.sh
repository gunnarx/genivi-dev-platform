...
If you still use it, you hereby agree to the applicable End-User License
Agreement (EULA) of the relevant BSPs that have them.
...
define_with_default() {
  ...
  default="$2"
  current_value=$(deref $var_name)
  echo "got: val $current_value default $default"
  if [[ -z "$current_value" ]]; then
...
if [[ -n "$FORK" ]]; then
  echo "***** NOTE: OVERRIDING REPOSITORY \$FORK DEFINITION *****"
...
stage_artifact cp gdp-src-build/tmp/deploy/images/*
stage_artifact cp gdp-src-build/conf/*
stage_artifact cp gdp-src-build/*/*.conf
